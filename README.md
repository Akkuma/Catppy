...
Catppy leverages React, Redux, a sprinkle of CSS & HTML.

The architecture is bootstrapped to speed up development. In a normal project I would create multiple components, but in this particular case the application was so simple that it seemed like overkill.

For a production app, I would use a css processor rather than handwriting it. I'd move more things into environment variables, such as the api key. I'd probably look to leverage alternative libs around Redux, such as Redux-Thunk, perhaps observables and a bunch of other quality of life improvements on a larger application that used Redux.

Catppy can be ran via `npm start` or `yarn` and can also be built in a production mode through `npm build`


## Create React Generated
